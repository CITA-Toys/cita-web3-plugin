...
const blockNumberFormatter = (num) => {
    if (typeof num === 'number') {
        return toHex(num);
    }
    else if (!(num.startsWith('0x') || num === 'latest' || num === 'earliest')) {
        return toHex(+num);
    }
    return num;
};
...
class CITA {
    constructor(server) {
        ...
        this.getBlockByNumber = ({ quantity, detailed, }) => this.citaFetch({
            ...
            params: [blockNumberFormatter(quantity), detailed],
        });
